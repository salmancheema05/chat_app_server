import { senderData,accepted,deleted,searchQuery } from "../models/requestModel.js";
...
const sendRequest = async (req,res)=>{
    try{
        const data = req.body
        const result =  [data.sender_id,data.receiver_id,'pending']
        console.log(result)
    }
    catch(error){
        res.status(500).send({'error':"server error"})
...
export {receiveRequest,acceptedrequest, deleteRequest,searchPeople,sendRequest }